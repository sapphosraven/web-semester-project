...
router.get('/', async (req, res) => {
    try {
        const orders = await OrderModel.find().populate('user', 'username').populate('items.shopItem'); 
        res.json(orders);
    } catch (err) {
    ...
    try {
        ...
        for (const item of order.items) {
            ...
            if (!shopItem || shopItem.stock < item.quantity) {
                return res.status(400).json({ message: 'Insufficient stock for item: ' + shopItem.name });
            }
